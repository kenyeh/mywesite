...
var BlockquoteButton, BoldButton, Button, CodeButton, CodePopover, ColorButton, Formatter, HrButton, FlickrButton, ImageButton, ImagePopover, IndentButton, Indentation, InputManager, ItalicButton, Keystroke, LinkButton, LinkPopover ,FlickrButton, FlickrPopover , ListButton, OrderListButton, OutdentButton, Popover, Selection, Simditor, SourceButton, StrikethroughButton, TableButton, TitleButton, Toolbar, UnderlineButton, UndoManager, UnorderListButton, Util,
    ...
    if ($node.is(this._allowedTags.join(',')) || isDecoration) {
      ...
      */
      ...
      if ($node.is('a') && ($childImg = $node.find('img')).length > 0) {
        ...
        $node.find('img').addClass("img-responsive");/*add responsive*/
      }
...
FlickrButton = (function(superClass) {
  extend(FlickrButton, superClass);

  function FlickrButton() {
  ...
  FlickrButton.prototype.render = function() {
    var args;
    args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    FlickrButton.__super__.render.apply(this, args);
    return this.popover = new FlickrPopover({
      button: this
    });
  };
  ...
  FlickrButton.prototype.status = function($node) {
    var showPopover;
    if ($node != null) {
      this.setDisabled($node.is(this.disableTag));
    }
  ...
  FlickrButton.prototype.command = function() {
    var $contents, $endBlock, $link, $newBlock, $startBlock, endNode, linkText, range, startNode, txtNode;
    range = this.editor.selection.getRange();
    if (this.active) {
      ...
      this.popover.one('popovershow', (function(_this) {
        return function() {
          if (linkText) {
            ...
            _this.popover.textEl.focus();
            return _this.popover.textEl.select();
          }
        };
      })(this));
    }
...
FlickrPopover = (function(superClass) {
  extend(FlickrPopover, superClass);

  function FlickrPopover() {
    return FlickrPopover.__super__.constructor.apply(this, arguments);
  }
  ...
  FlickrPopover.prototype.render = function() {
    ...
    */
    ...
    this.figcaptionEl.on('blur', (function(_this) {
      return function(e) {
        ...
        if(_this.figcaptionEl.val().length > 0)
        {
          return _this.target.html(_this.target.html()+"<figcaption>"+_this.figcaptionEl.val()+"</figcaption>");
        }else
        {
          return;
        }
    ...
    */
    ...
    return this.unlinkEl.on('click', (function(_this) {
      return function(e) {
        var range, txtNode;
        txtNode = document.createTextNode(_this.target.html());
        _this.target.replaceWith(txtNode);
        _this.hide();
        range = document.createRange();
        _this.editor.selection.setRangeAfter(txtNode, range);
  ...
  FlickrPopover.prototype.show = function() {
    var args;
    args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    ...
    this.target.html(html_val);
    
    return this.textEl.val(this.target.html());
  };

  return FlickrPopover;

})(Popover);


Simditor.Toolbar.addButton(FlickrButton);
